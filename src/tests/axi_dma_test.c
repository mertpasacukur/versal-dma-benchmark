                                   ...
                                   bool use_sg, TestResult_t* result)
{
    ...
    for (i = 0; i < warmup; i++) {
        ...
        if (status != DMA_SUCCESS) {
            LOG_ERROR("Warmup iter %lu: transfer failed with %d\r\n", (unsigned long)i, status);
            return status;
        }
        status = axi_dma_wait_complete(DMA_TIMEOUT_US);
        if (status != DMA_SUCCESS) {
            LOG_ERROR("Warmup iter %lu: wait failed with %d\r\n", (unsigned long)i, status);
            return status;
        }
    }
...
int axi_dma_test_throughput(MemoryRegion_t src_region, MemoryRegion_t dst_region, TestResult_t* result)
{
    ...
    if (size > AXI_DMA_MAX_TRANSFER_SIZE) {
        size = AXI_DMA_MAX_TRANSFER_SIZE;
    }

    LOG_DEBUG("AXI DMA Test: src_region=%d, dst_region=%d, size=%lu\r\n",
              (int)src_region, (int)dst_region, (unsigned long)size);
    ...
    LOG_DEBUG("AXI DMA Test: src_addr=0x%llX, dst_addr=0x%llX\r\n",
              (unsigned long long)src_addr, (unsigned long long)dst_addr);

    if (src_addr == 0 || dst_addr == 0) {
        LOG_ERROR("AXI DMA Test: Invalid address! src=0x%llX, dst=0x%llX\r\n",
                  (unsigned long long)src_addr, (unsigned long long)dst_addr);
