#===============================================================================
# Versal DMA Benchmark - Complete Design with All DMA Types (v3)
# VPK120 Board with LPDDR4
#
# v3 Changes:
#   - Removed M_AXI_LPD attempt (doesn't work for ADMA)
#   - Reduced NoC bandwidth requirements to 1000 MB/s
#   - ADMA access to LPDDR4 configured via CIPS internal settings
#   - AXI DMA increased to 26-bit length (64MB max)
#
# DMA Types:
#   - AXI CDMA: Memory-to-memory transfers
#   - AXI DMA: Stream-based transfers (with loopback FIFO)
#   - AXI MCDMA: Multi-channel stream DMA
#   - LPD DMA: PS hard IP (CIPS internal, configured for LPDDR4 access)
#===============================================================================
...
# Enable M_AXI_FPD for DMA control and interrupts
# Note: LPD DMA uses internal NoC paths configured in CIPS, not M_AXI_LPD
...
# Add 3 more slave interfaces for PL DMAs (CDMA, DMA, MCDMA)
...
# Configure new slave interfaces for PL DMAs
...
# Configure interfaces with realistic bandwidth (1000 MB/s = 1 GB/s each)
set_property -dict [list \
...
# Associate all PL interfaces with single PL clock
...
# Data paths: DMAs -> NoC
# CDMA has single M_AXI that handles both data and SG
...
if {[file exists $pdi_src]} {
    file copy -force $pdi_src "$export_dir/design_all_dma_v3.pdi"
    puts "PDI copied to: $export_dir/design_all_dma_v3.pdi"
}

# Generate XSA
set xsa_path "$export_dir/design_all_dma_v3.xsa"
...
puts "  - LPD DMA:   PS hard IP (uses CIPS internal NoC paths)"
puts ""
puts "Memory Map:"
puts "  LPDDR4: 0x0000_0000 - 0x7FFF_FFFF (2GB)"
puts ""
puts "v3 Changes:"
puts "  - Reduced NoC bandwidth from 5000 to 1000 MB/s per interface"
puts "  - Removed M_AXI_LPD (ADMA uses internal CIPS paths)"
puts "  - AXI DMA buffer length: 26-bit (64MB max)"
puts ""
puts "NOTE: LPD DMA (ADMA) accesses LPDDR4 through internal CIPS NoC paths."
puts "      If ADMA still can't access LPDDR4, use OCM (0xFFFC0000) for tests."
puts ""
