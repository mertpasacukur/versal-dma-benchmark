#===============================================================================
# Versal DMA Benchmark - Complete Design with All DMA Types (v2)
# VPK120 Board with LPDDR4
#
# v2 Changes:
#   - Fixed LPD DMA (ADMA) access to LPDDR4 via NoC
#   - Fixed AXI CDMA NoC configuration
#   - Increased AXI DMA buffer length from 14-bit to 26-bit (64MB max)
#
# DMA Types:
#   - AXI CDMA: Memory-to-memory transfers
#   - AXI DMA: Stream-based transfers (with loopback FIFO)
#   - AXI MCDMA: Multi-channel stream DMA
#   - LPD DMA: PS hard IP (enabled in CIPS, with NoC access to LPDDR4)
#===============================================================================
...
#===============================================================================
# CIPS - With LPD DMA enabled and LPD AXI master for NoC access
#===============================================================================
...
# Enable M_AXI_FPD, M_AXI_LPD, and LPD DMA with interrupts
# M_AXI_LPD provides the path for LPD components (including ADMA) to access PL/NoC
...
# Add 4 more slave interfaces: 3 for PL DMAs + 1 for LPD master (ADMA access)
...
# Configure new slave interfaces
...
# Configure PL DMA interfaces with improved bandwidth settings
set_property -dict [list \
...
# Configure LPD interface for ADMA access to LPDDR4
set_property -dict [list \
    CONFIG.CONNECTIONS {MC_0 {read_bw {5000} write_bw {5000} read_avg_burst {4} write_avg_burst {4}}} \
    CONFIG.CATEGORY {ps_rpu} \
] [get_bd_intf_pins $noc_cell/$si_lpd]

# Associate interfaces with clocks
...
#===============================================================================
# AXI CDMA - Memory to Memory (with fixed NoC settings)
#===============================================================================
...
#===============================================================================
# AXI DMA - Stream based with loopback (26-bit buffer length for 64MB max)
#===============================================================================
puts "Adding AXI DMA with 26-bit buffer length..."
create_bd_cell -type ip -vlnv xilinx.com:ip:axi_dma:* axi_dma_0

# CRITICAL: c_sg_length_width set to 26 bits = max 64MB per transfer
set_property -dict [list \
    CONFIG.c_include_sg {1} \
...
# 3 inputs: M_AXI_MM2S, M_AXI_S2MM, M_AXI_SG
...
# LPD to NoC SmartConnect (for ADMA access to LPDDR4)
...
# NoC LPD clock (use same PL clock for simplicity, or could use LPD clock from CIPS)
...
#-------------------------------------------------------------------------------
# LPD to NoC path (for ADMA/LPD DMA access to LPDDR4)
#-------------------------------------------------------------------------------
...
# Copy PDI file
set pdi_src "$project_dir/$project_name/$project_name.runs/impl_1/design_1_wrapper.pdi"
if {[file exists $pdi_src]} {
    file copy -force $pdi_src "$export_dir/design_all_dma_v2.pdi"
    puts "PDI copied to: $export_dir/design_all_dma_v2.pdi"
}

# Generate XSA
set xsa_path "$export_dir/design_all_dma_v2.xsa"
...
puts "XSA: $xsa_path"
puts "PDI: $export_dir/design_all_dma_v2.pdi"
puts ""
puts "DMA Types in Design:"
puts "  - AXI CDMA:  Memory-to-memory (SG mode)"
puts "  - AXI DMA:   Stream MM2S/S2MM with loopback FIFO, 26-bit length (64MB max)"
puts "  - AXI MCDMA: 4-channel stream with loopback FIFO"
puts "  - LPD DMA:   PS hard IP with NoC path to LPDDR4"
puts ""
puts "Memory Map:"
puts "  LPDDR4: 0x0000_0000 - 0x7FFF_FFFF (2GB)"
puts ""
puts "v2 Fixes:"
puts "  - LPD DMA (ADMA) now has M_AXI_LPD -> NoC -> LPDDR4 path"
puts "  - AXI CDMA burst size reduced to 64 for better NoC compatibility"
puts "  - AXI DMA buffer length increased from 14-bit to 26-bit"
puts ""
